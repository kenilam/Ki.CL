...
const WhatIGaveUp: React.FunctionComponent = () => (
  <>
    ...
    <Text is='p'>
      ...
      shows you one taxon, its lineage back to the origin, and a sampled fan of
      its neighbours.
    </Text>

    <Text is='p'>
      Three smaller compromises came with that. Illustrations are made on demand
      instead of drawn in advance, so most taxa have no plate until someone
      visits them and a provider has quota going spare. Deep fans are sampled
      rather than drawn whole, so you see a representative subset. And the tree
      is assembled lazily from a cache sitting in front of a public API, which
      makes the first visit to an unexplored clade slow in a way a printed sheet
      never is.
    </Text>
