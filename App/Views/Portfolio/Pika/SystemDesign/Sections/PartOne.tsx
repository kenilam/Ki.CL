  ...
  return (
    <Layout autoFlow='row' gap='wide' justifyItems='stretch'>
      <section>
        ...
        <Text>
          The manifest buys three things. Because inputs are a typed schema, the
          client can render any App&apos;s form straight from the manifest, so
          there is no per-App frontend code to write or maintain. Because the
          workflow is a DAG over primitives, the orchestrator can checkpoint,
          retry, and parallelize steps without understanding what any particular
          App is for. And because versions are immutable — publishing writes a
          new row, running jobs stay pinned to whatever version they started on
          — rolling back is just moving a pointer.
        </Text>
        <Text>
          ...
          declares its argument schema, cost model, timeout profile, and which
          providers can serve it. When a new vendor model appears, we register a
          provider, and every App with a model-select input picks it up without
        ...
        <Text>
          ...
          regenerate work and how provenance gets displayed — and in Part 2 it
          quietly becomes the agent&apos;s memory of everything created in a
          session.
        </Text>
        ...
        <Text>
          ...
          solve, and hand-rolled DAG engines are where reliability bugs go to
          live. The manifest interpreter is a single generic workflow, which
          means launching a new App deploys no orchestration code at all.
        </Text>
        <Text>
        ...
        <Text>
          <strong>Async everywhere versus a sync fast-path.</strong> I went back
          and forth on giving cheap image calls a synchronous endpoint and
          decided against it. Even fast models have p99s ugly enough to stall
          HTTP connections, and a second execution path is a second set of
          failure modes to reason about forever. So everything returns a job id
          and streams events. The UX concession is perceived sync — the client
          keeps the composer open and renders progress as it arrives.
        </Text>
        <Text>
          <strong>Declarative manifests versus Apps-as-code.</strong> The
          code-first version of this platform — every App its own service —
        ...
        <Text>
          <strong>Temporal versus hand-rolling.</strong> Hand-rolling a
          queue-plus-state-machine on Redis and Postgres is a fun six weeks
          followed by a career of edge cases — workers dying mid-callback, retry
          storms, clock skew on timeouts. Temporal buys durable timers,
          exactly-once state transitions, and workflow visibility off the shelf.
          For a company whose product is long-running jobs, that trade is
          clearly right, and lock-in stays manageable because workers only ever
          speak the task schema.
        </Text>
        <Text>
          <strong>Webhooks versus polling.</strong> Adapters prefer webhooks but
          keep a polling reconciler behind them, because webhooks get lost in
          the real world. Every provider call carries an idempotency key scoped
          to the logical task — stable across retry attempts — so a redelivered
          webhook or a re-scheduled attempt maps back to the same generation.
          And billing has its own guarantee: the ledger settles each task at
          most once, so a duplicate generation can never become a duplicate
          charge.
        </Text>
        ...
        <Text>
          With video generation, failure is not the exception to plan around —
          it is Tuesday. Providers time out, hand back corrupt files, and
          rate-limit without warning. So the design treats the individual task
          attempt, not the job, as the unit of failure.
        </Text>
        <List is='ul'>
          <ListItem>
            <Text dense>
              <strong>Retries with a taxonomy.</strong> Adapters classify every
              error: retryable gets exponential backoff with jitter, permanent
              fails fast and tells the user why, and a provider browning out
              trips a circuit breaker — failing work over to an equivalent
              provider where the manifest allows it.
            </Text>
          </ListItem>
          <ListItem>
            <Text dense>
              <strong>Checkpoint at every node.</strong> Task outputs persist
              before the next node schedules, so a crash resumes from the last
              completed step: a five-shot film that dies on shot five re-renders
              exactly one shot.
            </Text>
          </ListItem>
          <ListItem>
            <Text dense>
              <strong>Watchdogs and dead letters.</strong> Tasks that stop
              heartbeating are reaped and rescheduled; tasks that exhaust
              retries land in a dead-letter queue with full context, and jobs
              fail partially where the manifest allows — four of five shots plus
              a repair action, not an all-or-nothing error screen.
            </Text>
          </ListItem>
          <ListItem>
            <Text dense>
              <strong>Task-level monitoring.</strong> Every attempt is a row
              carrying app, primitive, provider, latency, cost and error class,
              so dashboards answer operational questions directly, and
              cost-anomaly alerts catch runaway loops before they bill anyone.
            </Text>
          </ListItem>
          <ListItem>
            <Text dense>
              <strong>Backpressure.</strong> Concurrency caps per user and per
              App, admission control keyed to queue depth, and the credit hold
              at job start — a viral App degrades to a queue position instead of
              toppling a provider.
            </Text>
          </ListItem>
        </List>
        ...
        <Text>
          Governance keeps self-serve from becoming self-harm: validation and a
          deliberately lightweight approval on publish, staged rollout with an
          automatic halt on error-rate or cost regression, one-click rollback
          via the version pointer, and per-App budgets to cap blast radius. The
          punchline is economic — because Apps are rows rather than deployments,
          the marginal cost of App #200 is a review, not a sprint.
        </Text>
      </section>
    </Layout>
  );
