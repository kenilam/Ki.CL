      ...
      <article className={`${CLASS_NAME}__article`}>
        <section>
          ...
          <Text is='p'>
            ...
            structural question completely: it shows you the shape of a
            phylogeny and how far apart its members sit.
          </Text>

          <Text is='p'>
            What it will not tell you is what any of them look like. Every tip
            is a name. One shows you the shape. The other shows you what lives
            at the ends of it. We wanted both on one page.
          </Text>
        </section>
        <Layout autoFlow='column' frames='9fr--3fr'>
          <section>
            <header>
              <Heading is='h2' className='kicl-font-size-large'>
              ...
              <Text is='p'>
                This is the thing we wanted to build. Every branch of life on
                one sheet, and at the end of every branch an organism you can
                actually recognise — a horse, a nautilus, a fly agaric — so that
                the shape of the tree and the creatures it produced are the same
                picture.
              </Text>
              ...
              <Text is='p'>
                ...
                cannot fetch them all. The interesting question stopped being
                “how do we draw the tree” and became “what do you show someone
                standing inside something that large”.
              </Text>
            </header>
        ...
        <section>
          ...
          <Text is='p'>
            ...
            <HyperLink to={toVersionPath({ version: '1' })}>v1</HyperLink>–
            <HyperLink to={toVersionPath({ version: '8' })}>v8</HyperLink>) drew
            into WebGL with the whole clade laid out at once and a cream
            backdrop borrowed from the poster. They looked like the reference
            and behaved nothing like it: past a few hundred taxa the layout
            collapsed into a mat of dots, and every one of them was the same
            handful of pixels wherever it sat.
          </Text>

          <Text is='p'>
            The middle versions (
            <HyperLink to={toVersionPath({ version: '9' })}>v9</HyperLink>–
            <HyperLink to={toVersionPath({ version: '13' })}>v13</HyperLink>)
            went the other way and became documents — DOM and SVG, tip-first,
            one clade at a time, drag and zoom instead of a fixed composition.
            They were legible and they were flat. You could read a name; you
            could not feel how far you were from the origin.
          </Text>
          ...
          <figure>
            ...
            <figcaption>
              <Text is='span' dense>
                ...
                — the map. Everything visible, nothing emphasised.
              </Text>
            </figcaption>
          </figure>
          ...
          <Text is='p'>
            ...
            the widest of them and made the trade explicit: showing everything
            means emphasising nothing.{' '}
            <HyperLink to={toVersionPath({ version: VERSION })}>
              v{VERSION}
            </HyperLink>{' '}
            gave that up. It frames one taxon and its immediate neighbourhood,
            treats the rest as context that fades into the air behind it, and
            lets the route — not a viewport — decide what you are looking at.
          </Text>
        </section>
        ...
        <section>
          ...
          <Text is='p'>
            ...
            neighbourhoods. Ask it for a node and you get what hangs below that
            node, to a height limit — which means the node you asked about
            arrives with no parent attached. Store that naively and you have
            written down that a genus is the root of all life.
          </Text>

          <Text is='p'>
            That is exactly what happened, and it took a while to see because
            the symptom was so far from the cause: <em>Panthera</em> rendered as
            the centre of the universe with nothing above it. So we stopped
            writing down “no parent” and “no parent yet” the same way. A cold
            fetch now also pulls the node’s rootward lineage — the whole spine
            back to the origin, including the unnamed internal nodes the
            synthetic tree strings between named ranks — so a stored null means
        ...
        <section>
          ...
          <Text is='p'>
            ...
            back. If the score fails, the prompt is tightened and it tries once
            more.
          </Text>
        </section>
        ...
        <section>
          ...
          <Text is='p'>
            ...
            a hundred images. That is not a model’s judgement, it is a hardcoded
            fallback: the vision provider had run out of credit, the call threw,
            and the pipeline substituted a passing score rather than admit it
            had not looked. A whole library shipped as “reviewed”.
          </Text>
          ...
          <Text is='p'>
            ...
            its head at an impossible angle to the neck, malformed fingers and a
            garbled caption scored a comfortable pass on morphology. Anatomy is
            now judged separately from identity, which is what makes “correct
            species, broken body” sayable at all.
          </Text>

          <Text is='p'>
            We took the bans out of the prompts. Naming something to forbid it
            just puts it in the picture: a list reading “no insects, no mites”
            produced microbes drawn as mites, and a fallback morphology ending
            “not a macroscopic animal” was quietly doing the same to every
        ...
        <section>
          ...
          <Text is='p'>
            ...
            non-retryable limit is put in cooldown and skipped, so the next one
            gets its turn rather than the request dying. Text, images and vision
          ...
          <Text is='p'>
            ...
            pays. Treating both as a thirty-minute cooldown meant retrying a
            spent account forever, twice an hour, and telling the reader to “try
            again shortly” for something only a billing page could fix. The two
            are now distinguished all the way to the panel, which says which
            kind of wait it is.
          </Text>
          ...
          <Text is='p'>
            ...
            arrives carrying an identity token minted by the site rather than by
            the browser.
          </Text>
        </section>
        ...
        <section>
          ...
          <Text is='p'>
            The poster shows every tip at once, illustrated, on one sheet.{' '}
            <HyperLink to={toVersionPath({ version: VERSION })}>
              v{VERSION}
            </HyperLink>{' '}
            shows one taxon, its lineage back to the origin, and a sampled fan
            of its neighbours. That is the trade. What made the poster beautiful
            is the same thing 2.3 million tips will not allow.
          </Text>
          ...
          <Text is='p'>
            ...
            lazily from a cache in front of a public API, so the first visit to
            an unexplored clade is slow in a way a printed sheet never is.
          </Text>

          <Text is='p'>
            What survived is the part worth keeping: you can{' '}
            <HyperLink to={toVersionPath({ version: VERSION })}>
              start at the origin of life
            </HyperLink>{' '}
            and walk to any organism, and the walk itself shows you how far it
            is.
          </Text>
        </section>
      </article>
