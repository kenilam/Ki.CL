...
const CLASS_NAME = 'kicl--views--experiments--tree-of-life--v3';
...
const Branch: React.FunctionComponent<BranchProps> = ({ link }) => {
  const geometry = useMemo(() => {
    ...
    for (let ring = 0; ring < rings; ring += 1) {
      ...
      // Ease taper so trunks stay thick longer, then thin toward tips.
...
const Scene: React.FunctionComponent<SceneProps> = ({
  ...
  const cameraDistance = Math.max(18, height * 1.35);
  ...
  return (
    <>
      ...
      <Drei.OrbitControls
        ...
        target={[0, targetY, 0]}
        minDistance={4}
        maxDistance={Math.max(80, height * 4)}
        maxPolarAngle={Math.PI * 0.82}
      />
      <Drei.PerspectiveCamera
        makeDefault
        position={[0, targetY * 0.55, cameraDistance]}
        fov={40}
        near={0.1}
        far={400}
      />
...
const Canvas: React.FunctionComponent = () => {
  ...
  return (
    <div className={CLASS_NAME}>
      ...
      >
        Even neighbor spacing · click glowing tips to grow a clade
      </Text>
    </div>
  );
};
