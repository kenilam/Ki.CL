...
Yarn 4 (Berry) workspace repo. Top-level workspace is `App`, which itself is a nested Yarn workspace of `.client`, `.server`, `router`, `views`, `widgets`. Node >=24 required. Prefer the `make` targets (thin wrappers over the `yarn run` scripts):
...
- **Existing classes first**: style with the utility classes and component modifiers that already exist before writing SCSS. When a new style would be useful in more than one place, add it as a new utility set in `App/core/styles` rather than a one-off rule.
...
- **Widths come from the 12-column scale** (`App/core/styles/columns.scss`): pages use 12 columns (80rem) through `kicl-max-inline-size-columns-12` or `var(--kicl-columns-12)`, and narrower blocks use a smaller span (`Card size={1…12}`, or `Layout columns` with `span` on its children). No hand-picked `rem`/`ch` widths.
